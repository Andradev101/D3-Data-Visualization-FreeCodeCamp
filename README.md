 ...
 <script>
  document.addEventListener('DOMContentLoaded', function(){
    document.getElementById('sendMessage').onclick = function(){
      ...
      xhr.open('POST', url, true); //opens request
      xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8'); //sets the request content type
      xhr.onreadystatechange = function () { //event listener
        if (xhr.readyState === 4 && xhr.status === 201){// readyState 4 == success sratus 201 == success
          const serverResponse = JSON.parse(xhr.response);
          document.getElementsByClassName('message')[0].textContent = serverResponse.userName + serverResponse.suffix;
        }
      };
      const body = JSON.stringify({ userName: userName, suffix: ' loves cats!' });
      xhr.send(body); //send it to the body
      // Add your code above this line
    };
  });
</script>
